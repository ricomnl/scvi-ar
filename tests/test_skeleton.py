...
def test_scarmodel():    
    ...
    n_celltypes = 8
    np.random.seed(42)
    gen = scrnaseq(n_cells, n_celltypes, n_features, n_total_molecules=n_total_molecules)
    adata = gen.generate(dirichlet_concentration_hyper=1)

    # adata = anndata.read_h5ad(Path(__file__).parent.joinpath('data', 'sim_data.h5ad'))
    ...
    fig = scrnaseq.heatmap(adata, return_obj=True)
    plt.savefig(Path(__file__).parent.joinpath('results', 'heatmap.png'))
    plt.close()

    # native signals are ground truth from simulated dataset
    denoised_dist = np.mean([wasserstein_distance(adata.layers['native_signals'][i,:], adata.layers['denoised_signals'][i,:]) for i in range(adata.shape[0])])
    orig_dist = np.mean([wasserstein_distance(adata.layers['native_signals'][i,:], adata.X[i,:]) for i in range(adata.shape[0])])
